...
global Xtrs, Ytrs, node, view_type
Xtrs = list()
Ytrs = list()
node = list()
view_type = 'bathy'
...
class MyApp(QMainWindow, Ui_MainWindow):
    ...
    def set_all_value(self,params):
        ...
        if isinstance(params['thetaf'], list):
            thetaf = []
        ...
        if isinstance(params['thetab'], list):
            thetab = []
    ...
    def refresh_it_all(self):
        ...
        if hsm.max() < maxdepth:
            QMessageBox.critical(QWidget(), "No", "last depth must be > Max depth of {:f}".format(maxdepth))
            return
        ...
        self.vgrid = self.create_vgrid(maxdepth=np.ceil(self.gr.values.max()),
                                       ...
                                       theta_b=thetab, theta_f=thetaf,
                                       hsm=hsm, nv_vqs=nv_vqs, 
                                       rutgers=rutgers)

        self.vgrid.compute_zcor(self.gr.values, 
                                a_vqs0=a_vqs0, 
